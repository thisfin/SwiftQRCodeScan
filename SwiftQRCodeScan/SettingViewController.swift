...
class SettingViewController: ViewController {
    ...
    lazy var shockVariable: Variable<Bool> = Variable(UserDefaultsHelp.shared.shock)
    ...
    override func viewDidLoad() {
        ...
        let items = Observable.just([
            SectionModel(model: "", items: ["\u{f09b}  源代码", "\u{f370}  应用商店"]),
            SectionModel(model: "", items: ["\u{f1e0}  分享", "\u{f126}  版本"]),
        ...
        let dataSource = RxTableViewSectionedReloadDataSource<SectionModel<String, String>>(configureCell: { (dataSource, tableView, indexPath, element) -> UITableViewCell in
            ...
            return i + 1 == dataSource.sectionModels.count ? nil : " " // section 分隔
        })
        ...
        _ = self.shockSwitch.rx.isOn <-> shockVariable // 双向绑定
